...
function Hierarchy(props) {
    ...
    const { scale: configScale, level: configLevel = -1 } = getItem(`hierarchyConfig`) || {};
    ...
    const createTextTitleTempRecord = ({ pathId, visible, pid, path, ...rest }) => {
        const rowId = uuidv4();
        // 记录不是顶级且子级没有展开则先展开子级
        if (pathId.length > 0) {
            ...
            const node = { pathId: pathId.concat(rowId), rowId, pid, path: [0], ...rest };
            const tempNodes = nodes.concat([node]);
            setNodes(tempNodes);
        }
    };
    ...
    const updateMovedRecord = args => {
        ...
        if (src.path.length === 1) {
            ...
            srcNode.path = [...(targetNode?.path || []), targetNode?.children?.length];
            ...
            const srcNode = nodes.find(v => v?.rowId === src?.rowId);
            if (parentNode) {
                parentNode.children = parentNode.children?.filter(v => v !== src.rowId);
            }
            if (!targetNode?.children) {
                targetNode.children = [];
            }
    ...
    const renderContent = () => {
                ...
                <Fragment>
                    {nodes.length > 0 && (
                        <LayerTitle
                            ...
                            allowEdit={!disEdit}
                            layersName={_.isEmpty(levelNames) ? initLayerTitle() : levelNames?.map(v => v?.name)}
                                ...
                                return item.pid ? null : (
                                    <TreeNode
                                        ...
                                        onDelete={rowId => {
                                            ...
                                            if (delNode) {
                                                const parentNode = nodes.find(v => v.rowId === delNode.pid);
                                                if (parentNode) {
                                            ...
                                            const c_nodes = nodes?.filter(v => v?.rowId !== rowId && !v?.pathId.includes(rowId)); //[data.rowId];
                                            setNodes(c_nodes);
                                            setSaveDisabled(false);
                                        }}
        ...
        return (
            <RecordStructureWrap className="hierarchyViewWrap scl" ref={$wrapRef} style={{}}>
                {renderHierarchy()}
            </RecordStructureWrap>
        );
    };
